...
public class ProxyIpController extends BaseController {
    ...
    @GetMapping("/getProxyLisy/{size}")
    @ResponseBody
    public Result getProxyLisy(@PathVariable("size") Integer size){
        List proxyIpList = proxyIpRedisService.findAllByPageRt(0,size);
        if (proxyIpList != null && proxyIpList.size() > 0) {
            Result result=new Result();
            result.setCode(200);
            result.setMessage("success");
            result.setData(proxyIpList);
            return result;
        }
        return null;
    }
